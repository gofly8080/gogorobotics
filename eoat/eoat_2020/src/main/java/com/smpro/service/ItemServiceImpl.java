...
public class ItemServiceImpl implements ItemService {
	...
	public Workbook writeExcelItemList(ItemVo vo, String type,HttpSession session) {
		...
		int arrSize = loginType.equals("S")?31:22;
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				cell.add(ivo.getStatusCode().equals("Y")? "판매중":ivo.getStatusCode().equals("N")?"판매중지":ivo.getStatusCode().equals("H")?"가승인":"대량등록");//판매상태 :상태코드 (Y=판매중, N=판매중지, H=가승인, E=엑셀 대량등록) */
