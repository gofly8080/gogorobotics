	...
	public String getData(HttpServletRequest request, @PathVariable Integer orderSeq, @RequestParam(required=false) Integer seq, @RequestParam(required=false) Integer sellerSeq, Model model) {
		...
		for (int i = 0; i < detailList.size(); i++) {
			...
			if(tempVo.getStatusCode().equals("99")){
				...
				System.out.println("!!!!! tempVo.getSellPrice():"+tempVo.getSellPrice());
				System.out.println("!!!!! tempVo.getOrderCnt():"+tempVo.getOrderCnt());
				cancelAmt +=(tempVo.getSellPrice()*tempVo.getOrderCnt());//<<<<<<<<
			}
			try {
		...
		System.out.println("!!!!! cancelAmt:"+cancelAmt);
		System.out.println("!!!!! ovo.getTotalPrice():"+ovo.getTotalPrice());
