...
public class ItemExcelController {
	...
	public String checkExcel(@RequestParam String filepath, @RequestParam(required=false) Integer sellerSeq, HttpServletRequest request, Model model, @RequestParam Integer mallSeq) {
		...
		for (int i = 1; i < v.size(); i++) {
			// 오류를 검증한다
			errorList.addAll(getErrorList(i + 1, v.get(i), LIST_SIZE,("A".equals(session.getAttribute("loginType")))?0:OPTION_SIZE,mallSeq)); // i에 1를 더하는 이유는 idx는 0부터 시작하니까 제목행을 포함해서 줄번호가 1 더 증가하기 때문이다.
		}
...
//				if(itemOptionService.getSeq(optionList.get(i).getItemSeq()) <=0){
	...
	private List<String> getErrorList(int idx, ArrayList<Object> list, int LIST_SIZE, int OPSION_SIZE, int mallSeq) {
		...
		if (list.size() != (LIST_SIZE+OPSION_SIZE)) {
			errorList.add(idx + " 번째 행이 잘못 작성되었습니다. [" + (LIST_SIZE+OPSION_SIZE)+ "] 개의 항목이 있어야 하는데 [" + list.size()	+ "] 개의 항목이 검색되었습니다");
			return errorList;
		}
		...
		if (!StringUtil.isBlank(img2)) {
			img2 = Const.ITEM_IMAGE_PATH+img2;
		...
		if(OPSION_SIZE>0) {
			...
			if ("".equals(list.get(index))) {
				...
				try {
					...
					//setOptionPrice
					errorList.add(idx + " 번째 행:" + index + "번째 열: 가격 값이 잘못되어 있습니다.:. "+list.get(index));
				}
				index++;
				//saleprice
				if (!"".equals(list.get(index))) {
	...
	private ItemOptionVo optionMapper(ArrayList<Object> list, int LIST_SIZE, int OPTION_SIZE) throws ExcelOutOfBoundsException {
		...
		System.out.println("setFreeDeli:"+list.get(index));
		vo.setFreeDeli(""+list.get(index++));//freeDeli
		System.out.println("setEventAdded:"+list.get(index));
