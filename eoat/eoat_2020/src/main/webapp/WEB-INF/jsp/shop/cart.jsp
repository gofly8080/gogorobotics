    ...
    <tr {{if stockCount==="0"}}class="danger"{{/if}}>
        ...
        <td>
            <div class="input-group" style="width:88px;">
                <select id="count<%="${seq}"%>" name="count" value="<%="${count}"%>"  onchange="Cart.add(<%="${seq}"%>, this);" class="form-control w80" >
                    <option value="<%="${count}"%>"><%="${count}"%></option>

                    <c:forEach var="i" begin="0" end="10000">

                        {{if ${i}==count}} <option value="${i}" selected="selected">${i} </option>
                        {{else}} <option value="${i}">${i}</option>
                        {{/if}}

                        <%--<option value="${i}" <c:if test="${i} eq <%="${count}"%>">selected="selected"</c:if>>${i} ,<%="${count}"%></option>--%>
                    </c:forEach>
                </select>
