...
<%@ page import="java.security.MessageDigest" %><%
    ...
    String CST_MID              = mallVo.getPgId();                    //LG유플러스으로 부터 발급받으신 상점아이디를 입력하세요.
    ...
    String LGD_MERTKEY          = mallVo.getPgKey(); //상점MertKey(mertkey는 상점관리자 -> 계약정보 -> 상점정보관리에서 확인하실수 있습니다)
    ...
    String LGD_ENCODING         = "UTF-8";                                                                                 //구매자 아이디
...
<form method="post" name ="LGD_PAYINFO" id="LGD_PAYINFO" action="/shop/${mallId}/order/result" target="_self">
    ...
    <input type="hidden" name="LGD_RETURNURL"               id="LGD_RETURNURL"      value="<%= CURRENT %>">
    <input type="hidden" name="LGD_ENCODING"                id="LGD_ENCODING"       value="<%= LGD_ENCODING %>">
    <!-- 결과 처리 -->
